/**
 * Function Call Check (Canary) -- creates a function with Data ABI usage,
 ...
 *
 * This is the Canary-suite version of the Hector function-call check.
 * Runs as part of the full canary suite at GET /canary.
 *
...
const FUNCTION_NAME = "canary-call-test";
const FUNCTION_SOURCE = `function handle(input) {
  var msg = (input && input.msg) || "canary-ok";
  ...
  try {
    ...
    steps.push(
      stepFromResponse("POST .../functions/{id}/call (sync invoke)", callResp, 200, (data) => {
        ...
        if (output.echo !== "canary-check") {
          return `output.echo expected "canary-check", got "${output.echo}"`;
        }
